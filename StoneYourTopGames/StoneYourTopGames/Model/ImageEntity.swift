...
fileprivate struct AttributeName {
...
extension Image {
    func record(with game: GameEntity, completion: ((String?) -> Void)? = nil) {
        guard let entity = dataManager.getManagedObject(type: .image) as? ImageEntity else {
    ...
    func record(imageWith gameId: Int, completion: ((String?) -> Void)? = nil) {
        ...
        if fetchResult.items == nil {
            ...
            if var entities = fetchResult.items as? [ImageEntity], gameId > 0 {
